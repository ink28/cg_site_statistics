...
SUM(cg_game_transaction_item.weight) As вес_мусора
FROM cg_game_transaction_item

JOIN cg_game_transaction
ON cg_game_transaction.Id = cg_game_transaction_item.transactionId
JOIN cg_game
ON cg_game_transaction.gameid = cg_game.Id
JOIN cg_user
ON cg_user.id = cg_game.creatorId

WHERE cg_game_transaction.deletedAt is null
and NOT cg_user.firstname IN ('Чистые') 
and cg_game_transaction.deletedAt is null
and cg_game.name != "Охота на колёса%"
and cg_user.id != "5a236fc0-0b4a-11ec-a692-a75f5615a0a7";
'''
